...
New covid cases per day in Medford (seven day average in red) [*temporary issue in year until Medford updates its data for 2022*]

```{r, echo=FALSE, message=FALSE, warning=FALSE}	
plot_cases <- ggplot(medford_from_city, aes(x=DATE, y=`New Cases`)) + geom_line(color="darkgray") + geom_line(aes(y=zoo::rollmean(`New Cases`, 7, na.pad=TRUE, align="right")), color="red")+ theme_light() + xlab("Date") + ylab("Number of new cases per day") + ggtitle("New covid cases in Medford per day")
print(plot_cases)
```
...
We can also see what percentage of people who have been hospitalized for covid had been fully vaccinated first. Vaccination is *very, very* effective at reducing severe illness and is strongly recommended by health agencies, but just as airbags and seatbelts reduce but do not completely eliminate car deaths, there is a small risk of a breakthrough infection that can lead to hospitalization. It is also not clear what fraction of "fully vaccinated" patients were boosted. Moreover, the fact that older people are more likely to require hospitalization and more likely to have been vaccinated will tend to make vaccination seem less effective than it is: to do a better comparison, one would have to look at hospitalization by vaccination status and age, and perhaps by other factors that may lead to worse outcomes with covid and may correlate with desire to get vaccinated in the first place.
